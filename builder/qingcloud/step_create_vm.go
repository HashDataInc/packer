...
func (step *StepCreateVM) Cleanup(state multistep.StateBag) {
	...
	if ok {
		...
		_, _ = instanceService.TerminateInstances(
			&service.TerminateInstancesInput{Instances: []*string{service.String(instanceID)}})
		_, _ = client.WaitInstanceStatus(instanceService, instanceID, "terminated",
			DefaultTimeout, DefaultInterval)
		_, _ = instanceService.CeaseInstances(
			&service.CeaseInstancesInput{Instances: []*string{service.String(instanceID)}})
	}
}
