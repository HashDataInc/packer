...
func (step *StepShutDownVM) Run(ctx context.Context, state multistep.StateBag) multistep.StepAction {
	...
	_, err = instanceService.StopInstances(
		&service.StopInstancesInput{Instances: []*string{service.String(instanceID)}})
	if err != nil {
		ui.Error(err.Error())
		return multistep.ActionHalt
	}

	_, err = client.WaitInstanceStatus(instanceService, instanceID,
		client.InstanceStatusStopped, DefaultTimeout, DefaultInterval)
	if err != nil {
		ui.Error(err.Error())
		return multistep.ActionHalt
	}
